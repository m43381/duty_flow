    ...
    def allowed_duty_types_for_clearance(self):
        ...
        if choice_rule:
            ...
            if choice_rule.mode == "specific_units":
                return qs.filter(created_by_unit_id__in=list(explicit_ids)).order_by("name")
            ...
            if choice_rule.mode == "scope_plus_units":
                ...
                return qs.filter(created_by_unit_id__in=list(scoped_ids | explicit_ids)).order_by("name")

            if choice_rule.mode == "scope":
                return filter_queryset_by_scope(self.ctx, qs, self.DUTY_TYPE_UNIT_LOOKUP, choice_rule.scope).order_by("name")
